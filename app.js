...
window.showBadgeInfo = function(type) {
    ...
    if (type === 'secure') {
        ...
        if (!isReturnable || isDropItem) {
            // Текст для вещей БЕЗ ВОЗВРАТА
            text = 'NISHA выступает гарантом сделки. Ваши деньги надежно защищены.<br><br>Данная вещь продается <b style="color:var(--accent-red);">без права на возврат</b>. Мы требуем полностью проверять и примерять её при получении в отделении Новой Почты.<br><br>Если вещь вам не подошла — вы просто отказываетесь от посылки на месте.';
        } else {
