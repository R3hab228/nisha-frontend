...
// 2. Сжатие фото (С защитой от переполнения памяти и ошибок)
async function compressImage(file) {
    // ФИКС 1: Пропускаем ВООБЩЕ ЛЮБЫЕ видео, а не только mp4
    if (file.type.startsWith('video/')) return file;
    
    // Добавили reject, чтобы скрипт мог выбросить ошибку и не висеть бесконечно
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            // ФИКС 2: Если формат кривой (HEIC или битый файл), отменяем сжатие
            ...
            img.onload = () => {
                ...
                canvas.toBlob((blob) => {
                    if (!blob) {
                        reject(new Error("Ошибка компрессии"));
                        return;
                    }
                    resolve(new File([blob], file.name, { type: 'image/jpeg' }));
                    
                    // ФИКС 3: ЖЕСТКАЯ ОЧИСТКА ПАМЯТИ ТЕЛЕФОНА!
                    img.src = ''; 
                    canvas.width = 0; 
                    canvas.height = 0;
                }, 'image/jpeg', 0.7);
