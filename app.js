    ...
    try {
       ...
       if (typeof autoAnimate === 'function') {
        ...
        if (_supabase) {
         ...
         // --- ЖИВОЕ ОБНОВЛЕНИЕ ЛЕНТЫ (БЕЗ ПЕРЕЗАГРУЗКИ) ---
            _supabase.channel('public:items')
                .on('postgres_changes', { event: '*', schema: 'public', table: 'items' }, async (payload) => {
                    ...
                    if (payload.eventType === 'INSERT') {
                        ...
                        await loadAllItems(); // Заново скачиваем базу и перерисовываем ленту!
                    } 
                    // ЕСЛИ АДМИН УДАЛИЛ ВЕЩЬ
                    else if (payload.eventType === 'DELETE') {
                        await loadAllItems(); 
                    }
                    // ЕСЛИ ВЕЩЬ КУПИЛИ ИЛИ ОБНОВИЛИ (Изменилась цена, статус)
                    else if (payload.eventType === 'UPDATE') {
                        const updatedItem = payload.new;
                        
                        // Если вещь забрали - показываем уведомление всем, кто на сайте
                        if (updatedItem.status === 'sold') {
                            showToast(`Только что забрали: ${updatedItem.name}`, 'error');
                            // Закрываем окно товара, если юзер прямо сейчас смотрит на эту вещь
                            if (currentOpenedItem && currentOpenedItem.id === updatedItem.id) {
                                closeModal('productModal');
                            }
                        }

                        // Если вещь сняли с брони (таймер вышел) - удаляем из корзин тех, кто не успел оплатить
                        if (updatedItem.status === 'available') {
                            ...
                            if (cartIdx !== -1) {
                                ...
                                syncCartToServer();
                                updateCartUI();
                                showToast(`Время брони вышло. ${updatedItem.name} удалена из корзины.`, 'error');
                            }
                        }
                        
                        // Полностью обновляем ленту, счетчики и кэш
                        await loadAllItems(); 
                    }
                })
                .subscribe();
...
function renderNextBatch() {
    ...
    // --- ОПТИМИЗАЦИЯ: Создаем невидимый контейнер в памяти ---
...
<button class="grid-cart-btn" data-i18n="product.add_to_cart" style="${item.status === 'sold' ? 'display:none;' : ''}" onclick="addToCartWithAnimation('${item.id}', this, event)">${i18next.t('product.add_to_cart')}</button>
    ...
    // --- ОПТИМИЗАЦИЯ: Вставляем ВСЕ карточки на страницу ОДНИМ действием! ---
            ...
            if (isVideo) {
                ...
                wrapper.innerHTML += `
                    <a href="${url}" data-pswp-width="1000" data-pswp-height="1000" target="_blank" class="slide skeleton" style="background-image:none; display:flex; align-items:center; justify-content:center; border: 1px solid #222;">
                        <img src="${url}" style="width:100%; height:100%; object-fit:contain; opacity:0; transition:opacity 0.4s ease-in-out;" 
...
async function submitProposal() {
    ...
    const rawDesc = document.getElementById('propDesc').value.trim(); // НОВОЕ ПОЛЕ ОПИСАНИЯ
    ...
    const desc = typeof DOMPurify !== 'undefined' ? DOMPurify.sanitize(rawDesc) : rawDesc; // НОВОЕ
    ...
    // --- ЖЕСТКАЯ ПРОВЕРКА РАЗМЕРА ---
    const cleanSize = size.toUpperCase();
    const isLetters = /^[A-Z]{1,4}$/.test(cleanSize); // Буквы от 1 до 4 (S, M, XL)
    const isNumbers = /^(3[0-9]|4[0-9])$/.test(cleanSize); // Цифры от 30 до 49
    const isUniversal = cleanSize === 'OS' || cleanSize === 'TU';

    if (!isLetters && !isNumbers && !isUniversal) {
        showToast('Размер: только буквы (S, XL) или цифры (30-49)', 'error');
        return;
    }
    ...
    try {
        ...
        fetch('https://nisha-api.onrender.com/api/propose', {
            ...
            body: JSON.stringify({
                name: nameItem,
                brand: brand,
                measurements: cleanSize, // Отправляем очищенный размер
                condition: cond,
                price: price,
                description: desc, // ОТПРАВЛЯЕМ ОПИСАНИЕ
                contact: contact,
...
// ==========================================
// ==========================================
// ИНФОРМАЦИОННЫЕ ОКНА ДЛЯ БЕЙДЖЕЙ ТОВАРОВ (МУЛЬТИЯЗЫЧНЫЕ)
// ==========================================
window.showBadgeInfo = function(type) {
    let title = '';
    let text = '';
    
    // Безопасное получение переводов (если i18next не загрузился, используем русские дефолты)
    const t = (key, defaultText) => {
        return typeof i18next !== 'undefined' ? i18next.t(key, { defaultValue: defaultText }) : defaultText;
    };

    const btnText = t('badge_info.btn_understood', '[ ПОНЯТНО ]');

    if (type === 'secure') {
        title = t('badge_info.secure_title', 'SECURE_PAYMENT.EXE');
        
        // Узнаем, можно ли вернуть текущий товар
        const isReturnable = currentOpenedItem && currentOpenedItem.is_returnable === true;
        const isDropItem = currentOpenedItem && (currentOpenedItem.is_drop === true || (currentOpenedItem.tags && currentOpenedItem.tags.map(tag => tag.toLowerCase()).includes('drop')));

        if (!isReturnable || isDropItem) {
            text = t('badge_info.secure_no_return_text', 'NISHA выступает гарантом сделки. Ваши деньги надежно защищены.<br><br>Данная вещь продается <b style="color:var(--accent-red);">без права на возврат или обмен ни при каких условиях</b>.<br><br>Мы настоятельно просим вас внимательно изучать фото, замеры и описание перед оформлением заказа.');
        } else {
            text = t('badge_info.secure_return_text', 'NISHA выступает гарантом сделки. Ваши деньги надежно защищены.<br><br>Вы можете примерить вещь на почте. Даже если вы забрали её домой, на данный товар действует <b style="color:var(--accent-green);">гарантия возврата и обмена в течение 14 дней</b>.<br><br><i>Обязательное условие возврата: сохранение товарного вида и отсутствие следов носки.</i>');
        }
    } else if (type === 'fast') {
        title = t('badge_info.fast_title', 'FAST_SHIPPING.SYS');
        text = t('badge_info.fast_text', 'Отправка заказа осуществляется в день оплаты (при подтверждении до 16:00) или на следующий рабочий день.');
    } else if (type === 'refund_no') {
        title = t('badge_info.refund_no_title', 'NO_RETURN_POLICY.LOG');
        text = t('badge_info.refund_no_text', '<span style="color:var(--accent-red); font-weight:bold; font-size:16px;">[ ТОВАР НЕ ПОДЛЕЖИТ ВОЗВРАТУ ]</span><br><br>Мы настоятельно просим вас внимательно изучать фото, замеры и описание перед оформлением заказа.<br><br><b style="color:var(--accent-red);">Данная вещь не подлежит возврату или обмену ни при каких условиях.</b>');
    } else if (type === 'refund_yes') {
        title = t('badge_info.refund_yes_title', 'RETURN_POLICY.SYS');
        text = t('badge_info.refund_yes_text', '<span style="color:var(--accent-green); font-weight:bold; font-size:16px;">[ ДОСТУПЕН ВОЗВРАТ ]</span><br><br>Данный товар подлежит возврату и обмену в течение <b>14 дней</b> с момента покупки, согласно законодательству Украины.<br><br><i>Условие возврата: сохранение товарного вида, всех бирок и отсутствие следов носки.</i>');
    } else if (type === 'drop') {
        title = t('badge_info.drop_title', 'WARNING: DROP_ITEM');
        text = t('badge_info.drop_text', '<span style="color:var(--accent-red); font-weight:bold; font-size:16px;">[ ВНИМАНИЕ ]</span><br><span style="color:#fff;">Эта вещь загружена сторонним продавцом (Creator).</span><br><br>Обязательно проводите полный осмотр вещи на отделении Новой Почты. <b style="color:var(--accent-red);">Если вы забрали посылку домой — возврат или обмен НЕВОЗМОЖЕН</b>, так как деньги сразу переводятся владельцу вещи.');
    }
